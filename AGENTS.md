...
## 行為準則
- Codex 可以依照使用者的任務卡，直接修改檔案、建立分支並提交 PR。
- 除非使用者特別要求「先寫計畫」，否則 Codex 不需要產生驗收計畫，可以直接執行。

## 任務卡格式
- 當使用者給出Codex指令區塊（明確列出檔名、完整內容、commit message），Codex 必須直接執行，不得只回覆文字。
- 若使用者僅給出模糊需求，Codex 需先提供計畫，再詢問是否執行。

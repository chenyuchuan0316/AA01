...
| glob（透過 Jest）                 | 7.x → 10.4.x                     | high     | `glob@10` 改為 Promise/async API，CLI 預設遵循現代 ignore/dot 行為，並去除 `inflight` 相依。                                                                                                  | `jest.config.cjs` 解析測試檔案的行為；任何以 glob pattern 搜尋檔案的自訂指令需確認 Node 18 行為一致。 | `npm test`，檢查測試發現器是否仍能鎖定 `__tests__/*.test.js`。                                      |
| jest                              | 29.7.0 → 30.2.0                  | medium   | 要求 Node ≥18；整合 `glob@10`、新的 CLI signal 處理，預設使用 `jest-circus` 並更新 watch 模式依賴。                                                                                           | `jest.config.cjs`、所有單元測試。                                                                     | `npm test -- --runInBand`。                                                                         |
...
- **Puppeteer**：目前已改採 Playwright 煙測並移除依賴；若未來重新導入需固定 `headless: true` 並預先安裝 `libnss3` 與字型後再調整等待邏輯。
...
- CI 需跑 `npm test` 與 `npm run e2e`（遠端 Playwright 規格會在缺少 `HAS_AUTH=true` 或憑證時自動 skip）。
